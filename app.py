...
<style>
    ...
    .stTabs [aria-selected="true"] {
        ...
        color: #ffffff !important;
        font-weight: 600;
        border-bottom: 2px solid #30c85a !important;
    }
    ...
    .stAlert {
        ...
        border-left: 3px solid #30c85a !important;
    }
...
with st.sidebar:
    ...
    col_bm1, col_bm2 = st.columns([2, 1])
    with col_bm1:
        selected_benchmark = st.selectbox("지수 선택", list(benchmark_dict.keys()), label_visibility="collapsed")
    with col_bm2:
        if st.button("추가", key="add_benchmark", use_container_width=True):
            bm_ticker = benchmark_dict[selected_benchmark]
            current = st.session_state.get('current_tickers', '')
            
            if bm_ticker not in current:
                if current and current.strip():
                    st.session_state['current_tickers'] = current + f", {bm_ticker}"
                else:
                    st.session_state['current_tickers'] = bm_ticker
                st.success(f"{selected_benchmark} 추가!")
                st.rerun()
            else:
                st.warning("이미 있음")
    ...
    if not tickers:
        ...
        if not all_data:
            ...
            with tab1:
                ...
                fig.update_layout(
                    title=dict(text="누적 수익률 비교", font=dict(color='#ffffff')),
                    xaxis_title="날짜",
                    yaxis_title="수익률 (%)",
                    ...
                    paper_bgcolor='rgba(30, 30, 50, 0.5)',
            ...
            with tab3:
                fig = go.Figure()
                # 밝고 선명한 색상 사용
                bright_colors = ['#00d4ff', '#ff6b6b', '#4ecdc4', '#ffd93d', '#6c5ce7', '#a8e6cf']
                ...
                fig.update_layout(
                    title=dict(text="거래량 비교", font=dict(color='#ffffff')),
                    xaxis_title="날짜",
                    yaxis_title="거래량",
                    template="plotly_dark",
                    barmode='group',
